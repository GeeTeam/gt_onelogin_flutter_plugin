...
class OLCustomWidget {
  String?
      viewId; //自定义控件 ID，如果是 Button 类型的组件，在 onCustomWidgetsClick 回调中通过 这个ID 区分不同组件的点击事件
  OLCustomWidgetType? type;

  ///自定义控件类型，当前只支持 View, ImageView, TextView，Button，其中只有Button 可点击
  OLRect? rect; //控件位置，宽高
